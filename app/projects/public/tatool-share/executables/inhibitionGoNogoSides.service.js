...
tatool
  .factory('inhibitionGoNogoSides', [ 'executableUtils', 'dbUtils', 'timerUtils', 'stimulusServiceFactory', 'inputServiceFactory',
    ...
    GoNogo.prototype.createStimulus = function() {
      ...
      var stimulus = null;
      if (this.counter == inhibitionStimuli[0] && inhibitionStimuli.length > 0) {
        this.trial.inhibition.push(inhibitionStimuli.shift());
        inhibitionStimuli.shift();
        stimulus = this.createInhibitionStimulus();
      } else {
        if (this.randomisation === 'full-condition') {
      ...
      if (stimulus === null) {
        ...
        if (stimulus.stimulusPosition == 'center') {
          this.stimulusServiceCenter.set(stimulus);
          this.currentStimulusService = this.stimulusServiceCenter;
        }else if (stimulus.stimulusPosition === 'left') {
    ...
    GoNogo.prototype.createInhibitionStimulus = function() {
      // get random stimuliType with replacement
      var inhibitionStimulus = {stimulusType:"incongruent", stimulusValue:"black_arrow_both.png", stimulusValueType:"image",correctResponse:"", keyCode:"", stimulusPosition:"center", response:"",keyIndex: ""};
